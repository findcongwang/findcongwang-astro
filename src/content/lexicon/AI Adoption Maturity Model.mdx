...
## Scoring Structure: The Agentic Gradient™ as Two-Axis Measure

The Agentic Gradient™ moves beyond simple tier progression to become a two-dimensional measurement framework. This refinement recognises that **adoption breadth** and **integration depth** operate along separate axes with distinct dynamics.

- **Axis 1: Adoption (breadth)** measures how far up the three-tier progression a business has climbed. This axis tracks the deployment of agentic AI capabilities across organisational functions.
- **Axis 2: Integration (depth)** measures how effectively that adoption integrates into organisational processes. Key factors include process readiness, Policy as Code implementation, human-in-the-loop design, and system reliability.
- **Derived output: Scaling Leverage** is the business amplification that emerges from the interplay of Adoption and Integration. This result emerges from a business's position on the Adoption-Integration plane rather than operating as a third input axis.

<MarginNote n={3} color="blue" note="Complementarity theory grounds this two‑axis structure. Milgrom & Roberts demonstrated that supermodular systems yield disproportionate returns when components align. Brynjolfsson, Li & Raymond (2023) found generative AI productivity gains depend on organisational redesign. Dell'Acqua et al.'s 'jagged frontier' shows AI excels at some tasks while humans retain others; integration quality determines which tasks shift.">High Adoption with low Integration yields low or negative leverage: AI worsens some tasks without governance.</MarginNote> High Adoption with high Integration yields super-additive leverage: the system outperforms its components. The "gradient" is literally the Adoption-versus-Integration plane; Scaling Leverage is what a business's position on that plane produces.

Scaling Leverage differs from HI-Scaling™. Scaling Leverage measures raw amplification, which can be high even in a hollowed-out organisation where AI carries the workload. HI-Scaling is the virtuous form where high Scaling Leverage coincides with high human cognitive vitality (see the Hybrid Intelligence entry).
...
- Milgrom, P., & Roberts, J. *Complementarities and systems.* 1995, Journal of Economic Literature.
- Brynjolfsson, E., Li, D., & Raymond, L.R. *Generative AI at work.* 2023, NBER Working Paper.
- Dell'Acqua, F., et al. *Navigating the jagged technological frontier.* 2023, Harvard Business School.
